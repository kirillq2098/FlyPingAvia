...
#
# Полный гайд Timeweb: [DEPLOY_TIMEWEB.md](DEPLOY_TIMEWEB.md)
#
...
- [ ] HTTPS работает в браузере: `https://app.flyping.ru/`

## B. Приложение

- [ ] `.env`: `APP_ENV=production`
- [ ] `.env`: `WEBAPP_URL=https://app.flyping.ru` (без `/`, query, fragment; **не** `*.trycloudflare.com`)
...
- [ ] Публично: `curl -sf https://app.flyping.ru/api/ready` → `{"ready":true,…}`
...
- [ ] `/setmenubutton` → бот → текст кнопки → `https://app.flyping.ru`
